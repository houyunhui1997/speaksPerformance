...
.year-element {
  position: absolute;
  bottom: clamp(24%, 26vh, 30%);
  left: 50%;
  transform: translateX(-50%);
  width: clamp(38%, 38vw, 56%);
  height: auto;
...
.landing-page__enter {
  position: absolute;
  left: 50%;
  bottom: calc(clamp(4%, 6vh, 9%) + env(safe-area-inset-bottom));
  width: clamp(34%, 40vw, 48%);
  padding: 0;
  border: 0;
...
.skip-btn {
  position: absolute;
  right: calc(clamp(16px, 1.6vw, 30px) + env(safe-area-inset-right));
  top: calc(clamp(16px, 1.6vw, 30px) + env(safe-area-inset-top));
  z-index: 1000;
  padding: 0;
  border: 0;
  background: transparent;
