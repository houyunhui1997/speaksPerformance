...
import bgImage from '@/assets/hotspot/web/hotspot-bg-web.png'
import video from '@/assets/hotspot/h5/video.png'
import videoBg from '@/assets/hotspot/web/video-bg-web.png'
...
const RADIUS = 280
const VERTICAL_RADIUS = 300
const FOCAL_LENGTH = 1100
const BASE_SPEED_Y = 0.0019
const BASE_SPEED_X = 0.0005
...
.hotspot-card {
  ...
  @each $size, $dimension in $card-sizes {
    &--#{$size} {
      width: calc(list.nth($dimension, 1) * var(--hotspot-scale));
      height: calc(list.nth($dimension, 2) * var(--hotspot-scale));
    }
  }
